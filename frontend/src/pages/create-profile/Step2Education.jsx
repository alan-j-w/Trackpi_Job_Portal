...
const Step2Education = ({ formData, setFormData, handleChange, onNext, onBack }) => {
    ...
    // Search States (Initialize when opening modal or typing)
    ...
    // Filter Logic - Courses (API)
    useEffect(() => {
        const fetchCourses = async () => {
            try {
                // If search is empty, fetch defaults (empty query)
                const query = courseSearch || "";
                // Pass the selected education level if available
                ...
                setFilteredCourses(res.data.map(c => c.name));
            } catch (err) {
    ...
    useEffect(() => {
        const fetchUniversities = async () => {
            try {
                ...
                const res = await axios.get(
                    `http://universities.hipolabs.com/search?name=${universitySearch}`
                );

                const unis = res.data.slice(0, 50).map(u => u.name);
                setFilteredUniversities([...new Set(unis)]);
            } catch (err) {
        ...
        const timer = setTimeout(fetchUniversities, 500);
        return () => clearTimeout(timer);
    }, [universitySearch]);
    ...
    const openEducationModal = (index = null) => {
        if (index !== null) {
            ...
            // Initialize search inputs with existing data
    ...
    const handleEducationSave = () => {
        // Use custom course if "Other" is selected
        const finalCourse = educationForm.course?.startsWith("Other")
            ? educationForm.customCourse
            : educationForm.course;

        // Custom validation
        if (!educationForm.level || !educationForm.university || !finalCourse) {
        ...
        <div className="space-y-8 animate-fadeIn">
            ...
            <div className="space-y-4 mb-10">
                ...
                <div className="bg-[#0B0F19] rounded-2xl p-6 flex flex-col md:flex-row items-center justify-between text-white shadow-xl relative overflow-hidden group border border-[#1F2937]">
                    ...
                    <button onClick={() => openLanguageModal()} className="mt-4 md:mt-0 bg-[#FFB300] hover:bg-[#ffaa00] text-black font-bold py-2.5 px-6 rounded-lg text-sm transition relative z-10 shadow-[0_0_15px_rgba(255,179,0,0.3)] whitespace-nowrap">
                        Add Language
                    </button>
                </div>
            ...
            <div className="flex justify-between mt-10">
                <button onClick={onBack} className="border-2 border-transparent hover:border-black px-12 py-3 rounded-xl font-bold bg-white text-black transition shadow-sm">
                    Back
                </button>
                <button onClick={onNext} className="bg-[#FFB300] hover:bg-[#ffaa00] text-black font-bold px-12 py-3 rounded-xl shadow-lg transition transform hover:scale-105">
                    Next
                </button>
            </div>
                    ...
                    <div className="bg-white rounded-2xl p-8 w-full max-w-2xl shadow-2xl relative animate-scaleIn max-h-[90vh] overflow-y-auto">
                        ...
                        <div className="space-y-6">
                            ...
                            <div className="relative group">
                                ...
                                <div
                                    className="w-full bg-white border border-gray-100 rounded-xl px-4 py-3.5 flex items-center justify-between cursor-pointer shadow-[0_2px_8px_rgba(0,0,0,0.08)] hover:shadow-md transition"
                                    onClick={() => setShowEduLevelDropdown(!showEduLevelDropdown)}
                                >
                                    <input
                                        className="w-full outline-none text-gray-700 font-medium placeholder-gray-400 cursor-pointer"
                                        ...
                                        onClick={(e) => {
                                            e.stopPropagation();
                                            setShowEduLevelDropdown(!showEduLevelDropdown);
                                        }}
                                    />
                                    <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" className={`text-gray-400 transform transition-transform ${showEduLevelDropdown ? 'rotate-180' : ''}`}><path d="M6 9l6 6 6-6" /></svg>
                                </div>
                                    ...
                                    <div className="absolute top-full left-0 right-0 mt-3 max-h-60 overflow-y-auto z-30 p-1 space-y-2 no-scrollbar">
                                        {EDUCATION_LEVELS.map((level, idx) => (
                                            <div
                                                ...
                                                className="px-5 py-3.5 rounded-xl bg-white border border-gray-100 cursor-pointer text-sm font-bold text-gray-700 shadow-sm hover:shadow-md transition-all hover:bg-gray-50 hover:scale-[1.01]"
                                                onClick={() => {
                                                    setEducationForm({ ...educationForm, level });
                                                    setEduSearch(level);
                                                    setShowEduLevelDropdown(false);
                                                }}
                                            >
                            ...
                            <div className="relative group">
                                ...
                                {["10th", "12th"].includes(educationForm.level) ? (
                                    ...
                                    // Existing Searchable Dropdown for others
                                    <>
                                        <div
                                            className="w-full bg-white border border-gray-100 rounded-xl px-4 py-3.5 flex items-center justify-between cursor-pointer shadow-[0_2px_8px_rgba(0,0,0,0.08)] hover:shadow-md transition"
                                        ...
                                        >
                                            <input
                                                ...
                                                onChange={(e) => {
                                                    ...
                                                    // Handle manual typing if not 'Other'
                                                    if (!e.target.value.startsWith("Other")) {
                                                        setEducationForm({ ...educationForm, course: e.target.value });
                                                    }
                                            ...
                                            />
                                            <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" className={`text-gray-400 transform transition-transform ${showCourseDropdown ? 'rotate-180' : ''}`}><path d="M6 9l6 6 6-6" /></svg>
                                        </div>
                                        {showCourseDropdown && (
                            ...
                            {educationForm.course?.startsWith("Other") && (
                                <div className="mt-4 animate-fadeIn">
                                    ...
                                    <input
                                        ...
                                        onChange={(e) => {
                                            setEducationForm({ ...educationForm, customCourse: e.target.value });
                                        }}
                                        value={educationForm.customCourse || ""}
                                        autoFocus
                                    />
                                </div>
                            )}
                                ...
                                {["10th", "12th"].includes(educationForm.level) ? (
                                    ...
                                    <>
                                        ...
                                        >
                                            ...
                                            />
                                            <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" className={`text-gray-400 transform transition-transform ${showUniversityDropdown ? 'rotate-180' : ''}`}><path d="M6 9l6 6 6-6" /></svg>
                                        </div>
                                ...
                                <div className="flex items-center gap-4">
                                    ...
                                    <div className="flex-1 relative group">
                                        ...
                                        >
                                            ...
                                            <svg width="10" height="6" viewBox="0 0 10 6" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" className={`text-gray-400 transform transition-transform ${showStartYearDropdown ? 'rotate-180' : ''}`}><path d="M1 1L5 5L9 1" /></svg>
                                        </div>
                                    ...
                                    <div className="flex-1 relative group">
                                        ...
                                        >
                                            ...
                                            <svg width="10" height="6" viewBox="0 0 10 6" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" className={`text-gray-400 transform transition-transform ${showEndYearDropdown ? 'rotate-180' : ''}`}><path d="M1 1L5 5L9 1" /></svg>
                                        </div>
                                        {showEndYearDropdown && (
                                            <div className="absolute top-full left-0 right-0 mt-3 max-h-48 overflow-y-auto z-30 p-1 space-y-1 no-scrollbar bg-white rounded-xl border border-gray-100 shadow-xl">
                                                {YEARS.map((year) => (
